  ...
  server.get("/log_in",(req,res)=>{
    try{
      if(req.isAuthenticated()){
        ...
        res.sendFile(dir + "/public/html_files/qr_authentication/scanner.html");
      }
    }catch(err){
      console.error('Error fetching department data:', err);
      res.status(500).send('Internal Server Error');
  ...
  server.get("/sign_up",(req,res)=>{
    try{
      if(req.isAuthenticated()){
        ...
        res.sendfile(dir+"/public/html_files/register/index.html");
      }
    }catch(err){
      console.error('Error fetching department data:', err);
      res.status(500).send('Internal Server Error');
...
//     const auths = await axios.post(`${domain}decrypt-barcode`,{
//       "encryptedData":req.body.scannedData,
//       "password":req.body.paasword,
//       "regno":req.body.username
//     })

//     console.log(auths.data.auth);
//     console.log(req.isAuthenticated())

//     // if(auths.data.auth == true){
//     //   console.log("yes")
...
server.post("/registeration",async(req,res)=>{
  console.log(req.body)
  const sending = await axios.post(`${domain}generate-barcode`,{
    "name":req.body.name,
...
server.post("/opt_verify",async(req,res)=>{
    ...
    transporter.sendMail({
      ...
      to: `${req.body.email}`,
      subject: `Your OTP Code: ${otp}`,                  // list of receivers
      html: `
      <!DOCTYPE html>
  <html lang="en"> 
  <head>
  ...
  replyTo: 'zenotionsolutions@gmail.com'
      }, (error, info) => {
          if (error) {
...
server.post("/reset_otp",(req,res)=>{
    ...
    transporter.sendMail({
      ...
      subject: `Your OTP Code: ${otp}`,                          // Subject line
      ...
      replyTo: 'zenotionsolutions@gmail.com'            // HTML body content
      }, (error, info) => {
          if (error) {  
              return console.log(error);
          }
...
//summary section
// Function to parse .docx file buffer
...
/**
 * Formats raw text for structured display with proper spacing, titles, subtitles, and emphasis.
 * @param {string} input The input string to format.
 * @returns {string} The formatted text.
 */
const formatRawText = (input) => {
  ...
  const formattedLines = lines.map((line) => {
      if (line.startsWith('**') && line.endsWith('**')) {
          // Format bold section titles
          return `\n${line.slice(2, -2).toUpperCase()}\n${'='.repeat(line.length - 4)}\n`;
      } else if (line.startsWith('*') && line.endsWith('*')) {
          // Format subtitles (e.g., *Subtitle*)
          return `\n${line.slice(1, -1)}:\n${'-'.repeat(line.length - 2)}\n`;
      } else if (line.match(/\*\*(.*?)\*\*/g)) {
          // Format inline bold text
          return line.replace(/\*\*(.*?)\*\*/g, (_, boldText) => boldText.toUpperCase());
      } else {
          // Normal paragraphs with appropriate indentation
          return `  ${line}`;
      }
  });
...
function formatTextDynamically(text) {
  // Split text into lines for processing
  ...
  lines.forEach(line => {
      // Check for headings (e.g., UNIT or Sections)
      if (/^UNIT [IVXLCDM]+:/.test(line)) {
          isUnit = true;
          htmlContent += `<h2>${line}</h2>`;
      } else if (/^[A-Z ]+:/.test(line)) {
          // Check for other headings (e.g., CHAPTER, TOPIC)
          ...
          htmlContent += `<li>${line.replace(/^[•*-]/, "").trim()}</li>`;
      } else if (isUnit) {
          // Append paragraphs inside a unit
...
server.get("/:dept/:sem/:sub/:unit/:topic/document/advance/summary", async (req, res) => {
  const { dept, sem, sub, topic, unit } = req.params;
  const { source } = req.query;

  console.log(dept, sem, sub, topic, source);


  // Make the API request to get the PDF or DOCX data
  ...
  const issum = await axios.post(`${domain}show_pdf`, {
    "dept": dept,
    "sub": sub,
    "topic": topic,
    "unit": unit,
  ...
  // Extract the file extension to decide whether it's a PDF or DOCX
  let extension = source.split(".")[1];
  const bufferdata = Buffer.from(pdfResponse.data.data);
  let extractedText = '';
  try {
    if (extension === 'pdf') {
      extractedText = await extractTextFromPDFBuffer(bufferdata);
    } else if (extension === 'docx') {
      extractedText = await extractTextFromDocxBuffer(bufferdata);
    }
  } catch (err) {
    console.error('Error extracting text:', err);
  }
  const genAI = new GoogleGenerativeAI("AIzaSyAbzxa1yM6lax3QN2aBwIifVl7tRUG8rXY");
  const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

  const prompt = `Can you generate me summary on the content "${extractedText}" in short with about 1000 words in html formate so that it will be easy to display in my website without head section i need only body section it should be neatly aligned`;

  const result = await model.generateContent(prompt);
  let text = result.response.text();

  let final_text = formatRawText(text);
"  const formattedHTML = formatTextDynamically(final_text);"
let cleanText = final_text
                .replace(/html/g, '').trim()
                .replace(/^```|```$/g, '').trim(); 

  // Render the page with the extracted text
  res.render("resource_page/resource_advance.ejs", {
    title: "Summary",
    option: "summ",
    our_domain: our_domain,
    source: source,
    ...
    topic: topic,
    unit: unit,
    summary: cleanText // Pass the extracted text to the template
  });
});
...
server.get("/:dept/:sem/:sub/:unit/:topic/document/advance/qgen", async (req,res)=>{
  ...
  if (!questions[topic]) {
    ...
    // Prompt to generate MCQs in JSON format
    const prompt = `
    Please generate 10 MCQ questions on "${extractedText}" in this JSON format:
    [
      {
        "question": "",
    ...
    const result = await model.generateContent(prompt);
    const text = result.response.text(); // Adjust based on library's actual response
    console.log("Generated Raw Content:", text);
    ...
    // Store questions under the topic
    questions[topic] = parsedQuestions.map((q, index) => ({
        id: index + 1,
        question: q.question,
        options: q.options,
        answer: q.answer
    ...
    res.render("resource_page/resource_advance.ejs",{"title":"Question Genration","option":"qgen","our_domain":our_domain,"source":source,"dept":dept,"sem":sem,"sub":sub,"topic":topic,"unit":unit,"questions":questions.topic});
  }
})
  ...
  new Strategy(
    ...
    async function verify(req, username, password, cb) {
      try {
        ...
        const scannedData = req.body.scannedData; // Extract scanned data
        if (!scannedData) {
          return cb(null, false, { message: "Scanned data is required." });
        }
        ...
        if (auths.data.auth) {
          // Replace "err" with the correct condition for your logic
          const valid = true; // Adjust logic to match your response structure
          if (valid) {
            return cb(null, { username }); // Pass the user object
          } else {
